...
{
    float scale_cols = 160/(float)image.cols ;
    float scale_rows = 160/(float)image.rows ;
    float scale = scale_cols < scale_rows ? scale_cols : scale_rows; 
    
    int dst_cols = (int)(scale * image.cols);
    int dst_rows = (int)(scale * image.rows);

    cv::Mat dst_image;
    cv::resize(image, dst_image, cv::Size(dst_cols,dst_rows));
    //resize 
    cv::Mat letterboxImage(160, 160, CV_8UC3, (void*)input_data);
    
    //add black
    int x_offset = (160 - dst_cols) / 2;
    int y_offset = (160 - dst_rows) / 2;
    dst_image.copyTo(letterboxImage(cv::Rect(x_offset, y_offset, dst_cols, dst_rows)));
